...
do_install() { 
    ...
    install -m 0644 ${UNPACKDIR}/20-wlan0.network ${D}${sysconfdir}/systemd/network
    sed -i "s|wlan0|${WLAN_DEV}|g" ${D}${sysconfdir}/systemd/network/20-wlan0.network

    install -m 0644 ${UNPACKDIR}/wpa-conf.service ${D}/${systemd_unitdir}/system
    sed -i "s|wlan0|${WLAN_DEV}|g" ${D}/${systemd_unitdir}/system/wpa-conf.service

    install -m 0644 ${UNPACKDIR}/wpa-conf.timer ${D}/${systemd_unitdir}/system
    #install -m 0644 ${UNPACKDIR}/wpa_supplicant@.service ${D}/${systemd_unitdir}/system
}
